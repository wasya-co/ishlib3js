...
const useApi = () => {
  ...
  if (!token) {
    console.log("Warning: jwt_token is null.");
  }
  return {
    ...
    getLocation: props => {
      ...
      return request.get(`/api/maps/view/${slug}?jwt_token=${token}&newsitems_page=${_npg}`).then(r => r.data).then(r => {
        return r.map;
      }).catch(err => {
        return err;
      });
    ...
    initPayment: p => request.post(`${config.apiOrigin}/api/payments2`, {
      jwt_token: token,
    ...
    applicationHome: () => {
      request.get(`${config.apiOrigin}/api/sites/view/${config.domain}`, {
        params: {
          jwt_token: token
        }
      }).then(r => {
    ...
    vote: props => {
      ...
      return request.post(`${config.apiOrigin}/api/v1/vote/${votee_class_name}/${votee_id}/${voter_id}/${value}`, {
        params: {
          jwt_token: token
        }
      }).then(r => {
